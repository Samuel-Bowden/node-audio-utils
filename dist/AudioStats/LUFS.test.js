...
const AudioStats_1 = require("./AudioStats");
/** Generic so I can verify the rust implementation against the existing tests */
class LoudnessStatsGeneric {
}
function sendWave(stats, params) {
    ...
    let levelPerChannel = new Array(numChannels);
...
function sendSilence(stats, params) {
    const sampleRate = params.sampleRate ?? 48_000;
    const numChannels = params.numChannels ?? 2;
    const numSamples = Math.floor(sampleRate * params.durationMs / 1000);
    const totalSamples = numSamples * numChannels;
...
function createLoudnessStats(channels) {
    const stats = new AudioStats_1.AudioStats({
        sampleRate: 48000,
        channels,
        bitDepth: 16,
    });
...
describe('LoudnessStats', () => {
    ...
    it('5 minute test', () => {
        const loudnessStats = createLoudnessStats([3 /* Channel.Center */]);
        sendWave(loudnessStats, {
            numChannels: 1,
            frequency: 997,
            durationMs: 5 * 60_000,
        });
        (0, chai_1.expect)(loudnessStats.getStats().iLufs).to.be.approximately(-3.01, 0.001);
    }).timeout(60_000);
    ...
    it('Passes EBU Test case #1', () => {
        const loudnessStats = createLoudnessStats([1 /* Channel.Left */, 2 /* Channel.Right */]);
        sendWave(loudnessStats, {
            durationMs: 20_000,
            dBFS: -23.0,
        });
        (0, chai_1.expect)(loudnessStats.getStats().mLufs).to.be.approximately(-23.0, 0.1);
        (0, chai_1.expect)(loudnessStats.getStats().sLufs).to.be.approximately(-23.0, 0.1);
        (0, chai_1.expect)(loudnessStats.getStats().iLufs).to.be.approximately(-23.0, 0.1);
    });
    it('Passes EBU Test case #2', () => {
        ...
        (0, chai_1.expect)(loudnessStats.getStats().mLufs).to.be.approximately(-33.0, 0.1);
        (0, chai_1.expect)(loudnessStats.getStats().sLufs).to.be.approximately(-33.0, 0.1);
        (0, chai_1.expect)(loudnessStats.getStats().iLufs).to.be.approximately(-33.0, 0.1);
    });
    ...
    it('Passes EBU Test case #6', () => {
        const loudnessStats = createLoudnessStats([1 /* Channel.Left */, 2 /* Channel.Right */, 3 /* Channel.Center */, 4 /* Channel.LeftSurround */, 5 /* Channel.RightSurround */]);
        sendWave(loudnessStats, {
            durationMs: 20_000,
            numChannels: 5,
    ...
    it('Passes EBU Test case #11', () => {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            (0, chai_1.expect)(maxMLUFS).to.be.approximately(i - 38.0, 0.1);
        }
    }).timeout(4_000);
